  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        Swal.fire("Error!", "There was an error fetching the bookings.", "error");  // แสดงข้อความผิดพลาด
      } finally {
...
const handleEditBookingSubmit = async (e) => {
  ...
  try {
    ...
    // อัปเดตข้อมูลการจองในสเตตของแอป โดยการแทนที่ข้อมูลของการจองที่ถูกแก้ไข
    ...
    const errorMessage =
      error.response?.data?.message || "There was an error while updating the booking."; // ข้อความจาก API หรือข้อความผิดพลาดทั่วไป

    Swal.fire("Error!", errorMessage, "error"); // แสดงข้อความแสดงข้อผิดพลาด
  }
};
...
const handleDeleteClick = async (booking) => {
  ...
  Swal.fire({
    title: `Are you sure you want to delete booking for ${booking.field.name}?`, // ถามยืนยันว่าจะลบการจองของสนามนี้หรือไม่
...
const renderTimeSlots = () => {
  ...
  // แบ่ง timeSlots ออกเป็นแถวละ 5 ช่องเวลา
...
return (
  <div className="min-h-screen bg-gray-100 p-6">
    <div className="max-w-7xl mx-auto">
      ...
      {/* เมื่อคลิกจะเปิดฟอร์มการเพิ่มการจอง */}
      <div className="mb-6">
        <button
